#!/usr/bin/env bash
# download-and-inspect.sh — Download without installing, extract and thoroughly inspect
# Usage: download-and-inspect.sh <pypi|npm> <package> [version]
set -euo pipefail
...
if [ "$ECOSYSTEM" = "pypi" ]; then
  ...
  pip download --no-deps -d "$INSPECT_DIR" "$PIP_TARGET" 2>/dev/null || {
    echo '{"error":"Download failed","package":"'"$PACKAGE"'"}'
    rm -rf "$INSPECT_DIR"
    exit 1
  }
  ...
  if [ -n "$PTH_FILES" ]; then
    while IFS= read -r pth; do
      ...
      if echo "$CONTENT" | grep -qEi '(import |base64|exec|eval|subprocess|socket|http|urllib)'; then
        add_finding "pth-malicious" ".pth file contains executable code" "critical" "$pth"
      else
        add_finding "pth-present" ".pth file present (content appears benign)" "medium" "$pth"
      fi
    done <<< "$PTH_FILES"
  fi

  # 2. setup.py cmdclass override
  ...
  if [ -n "$SETUP_PY" ] && [ -f "$SETUP_PY" ]; then
    if grep -qE 'cmdclass\s*=' "$SETUP_PY" 2>/dev/null; then
      # Check if install/develop commands are overridden
      if grep -qE "(install|develop|egg_info)" "$SETUP_PY" 2>/dev/null; then
        add_finding "setup-cmdclass" "setup.py overrides install commands" "high" "$SETUP_PY"
      fi
    fi
    # Check for network calls in setup.py
    if grep -qEi '(requests\.|urllib|http\.client|socket\.|subprocess)' "$SETUP_PY" 2>/dev/null; then
      add_finding "setup-network" "setup.py contains network/process calls" "critical" "$SETUP_PY"
    fi
  fi

  # 3. __init__.py top-level network calls
  ...
  if [ -n "$INIT_FILES" ]; then
    while IFS= read -r init; do
      if grep -qEi '(requests\.post|urllib\.request\.urlopen|http\.client\.HTTP|socket\.connect)' "$init" 2>/dev/null; then
        add_finding "init-network" "__init__.py contains network calls" "high" "$init"
      fi
    done <<< "$INIT_FILES"
  fi

  # 4. exec/eval + encoding
  ...
  if [ -n "$EXEC_FILES" ]; then
    while IFS= read -r ef; do
      if grep -qEi '(base64|codecs\.decode|bytes\.fromhex|decode\()' "$ef" 2>/dev/null; then
        add_finding "obfuscated-exec" "exec/eval executes encoded content" "critical" "$ef"
      fi
    done <<< "$EXEC_FILES"
  fi

  # 5. Large base64 blobs
  B64_FILES=$(grep -rlE '[A-Za-z0-9+/]{200,}={0,2}' "$EXTRACT_DIR" --include="*.py" 2>/dev/null || true)
  if [ -n "$B64_FILES" ]; then
    add_finding "base64-blob" "Found large base64 encoded blob (>200 chars)" "high" "$(echo "$B64_FILES" | head -1)"
  fi

  # 6. Sensitive directory access
  SENSITIVE=$(grep -rnE '(\.ssh|\.aws|\.kube|\.gnupg|\.config/gcloud|credentials)' "$EXTRACT_DIR" --include="*.py" 2>/dev/null | head -5 || true)
  if [ -n "$SENSITIVE" ]; then
    add_finding "sensitive-access" "Found sensitive directory/file access patterns" "critical" ""
  fi

  # 7. Bulk environment variable reading
  ENV_COUNT=$(grep -rnE 'os\.environ' "$EXTRACT_DIR" --include="*.py" 2>/dev/null | wc -l || echo "0")
  if [ "$ENV_COUNT" -gt 10 ]; then
    add_finding "env-harvest" "Excessive environment variable reading (${ENV_COUNT} occurrences)" "high" ""
  fi

  # 8. Unusual binary files
  BIN_FILES=$(find "$EXTRACT_DIR" \( -name "*.so" -o -name "*.dll" -o -name "*.dylib" -o -name "*.exe" \) 2>/dev/null || true)
  if [ -n "$BIN_FILES" ]; then
    BIN_COUNT=$(echo "$BIN_FILES" | wc -l)
    add_finding "binary-files" "Contains ${BIN_COUNT} binary file(s)" "medium" ""
  fi
  ...
  # Scan package.json install scripts
...
" 2>/dev/null || echo "")
    if [ -n "$INSTALL_SCRIPTS" ]; then
      add_finding "install-scripts" "Found install hooks: $INSTALL_SCRIPTS" "high" "$PKG_JSON"
    fi
  fi
  ...
  if [ -n "$EXEC_FILES" ]; then
    add_finding "child-process" "Uses child_process to execute commands" "high" "$(echo "$EXEC_FILES" | head -1)"
  fi

  NET_FILES=$(grep -rlE '(net\.connect|http\.request|https\.request|fetch\()' "$EXTRACT_DIR" --include="*.js" 2>/dev/null || true)
  if [ -n "$NET_FILES" ]; then
    add_finding "network-calls" "Found network requests" "medium" "$(echo "$NET_FILES" | head -1)"
  fi

  B64_FILES=$(grep -rlE 'Buffer\.from\(.{50,},.*base64' "$EXTRACT_DIR" --include="*.js" 2>/dev/null || true)
  if [ -n "$B64_FILES" ]; then
    add_finding "base64-decode" "Found base64 decode operations" "high" "$(echo "$B64_FILES" | head -1)"
  fi

  EVAL_FILES=$(grep -rlE 'eval\s*\(' "$EXTRACT_DIR" --include="*.js" 2>/dev/null || true)
  if [ -n "$EVAL_FILES" ]; then
    add_finding "eval-usage" "Uses eval()" "high" "$(echo "$EVAL_FILES" | head -1)"
  fi
fi
